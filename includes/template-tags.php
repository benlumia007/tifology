...
================================================================================================
*/
function tifology_entry_posted_on() {
    $author_avatar_size = apply_filters( 'tifology_author_avatar_size', 100 );
    ...
    esc_attr( sprintf(__( 'View all posts by %s', 'tifology'), get_the_author() ) ), 
    get_the_author()
    ));

    if ( !is_page() && !post_password_required() && ( comments_open() || get_comments_number() ) ) {
        echo '<span class="entry-comments"><b>';
            comments_popup_link( sprintf( __( 'Leave a Comment', 'tifology') ) );
        echo '</b></span>';
    }
}

function tifology_index_entry_posted_on() {
...
================================================================================================
*/
function tifology_entry_taxonomies() {
    $cat_list = get_the_category_list(__( ' | ', 'tifology' ) );
    $tag_list = get_the_tag_list( '', __(' | ', 'tifology') );

    if ( $cat_list ) {
        printf( '<div class="cat-link"> %1$s <span class="cat-list"l><b><i>%2$s</i></b></span></div>',
        __( '<i class="fa fa-folder-open-o"></i> Posted In', 'tifology' ),  
        $cat_list
        );
    }
    ...
    if ( $tag_list ) {
        ...
        __( '<i class="fa fa-tags"></i> Tagged', 'tifology' ),  
        $tag_list 
        );
    }
}